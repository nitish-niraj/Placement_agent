...
**Amendment 2026-09-13 (owner decision, P13 scope):** the **form/document submission** side of KYC and placement handling is now IN scope, behind the approval workflow. The pipeline detects placement form requests (announcement text or a Google/MS/Glide Forms link) as FORM events with reminders, drafts a `form_draft` action pre-filled from the owner's stored profile **at read time only** (SEC-002: decrypted values are never persisted into the actions payload), and submission happens **only after the owner explicitly approves that exact draft on the dashboard** (ADR-008: nothing reaches the real world without approval). The actual submit executor is the first P14 capability and requires its own per-capability ADR (SEC-005). The line that does NOT move: **attendance at the post-shortlist evaluative session remains always manual** — no bot ever sits an evaluation in Nitish's place.

**Amendment 2026-09-22 (owner decision, P14.2 scope — manual Submit only):** the P14.1 headless submit executor is **retired from the approve path**. Approving a `form_draft` records the decision (audited) and nothing else. Filling happens visibly in the owner's own browser, two ways: (1) the dashboard expands the draft into the real form **pre-filled via entry-ID link** (Google Forms; server never fills or submits), and (2) a profile-baked **auto-fill bookmarklet** for login-walled forms the server cannot read (values-only, no token, no network). The owner reviews and clicks **Submit themselves, every time**. Rationale: same-origin policy makes server-side filling invisible and login walls make it impossible; owner-in-browser keeps the DEC-008 posture (nothing reaches the real world except by the owner's own hand) with strictly less machinery. Consequence: `routes/actions.py` approve no longer enqueues the robot; `pia_api/prefill.py` + dashboard Approvals own the flow; `automation/` remains as dormant fallback code.
...
- **Consequence:** 01_PRD non-goals and 02_TRD action-engine guardrails must state this explicitly; no worker doc may design KYC *attendance* automation. Submission automation exists only as approval-gated drafts + the P14 executor.
...
- **Superseded 2026-09-22 (P14.2, DEC-008 amendment above):** the executor is retired from the approve path — approval records the decision only; filling happens owner-in-browser (entry-ID pre-fill links + auto-fill bookmarklet). `automation/` stays as dormant fallback code.
...
### DEC-011 — Application-aware notifications: company mentioned != applied (2026-09-22)
- **Decision:** Notifications become application-aware instead of company-aware. A new `application_states` table tracks the student's own answer per company+role opportunity (`UNKNOWN / ELIGIBLE_NOT_APPLIED / APPLIED / NOT_APPLIED / NOT_SURE / NOT_INTERESTED`; fully user-driven machine, pipeline only creates neutral rows, every change audited). Post-application-shaped messages (action/confirmation tied to an application, deterministic classifier in `notify/classify.py`) become follow-ups ONLY when the matched company+role row is APPLIED (role mismatch never matches); undecided states get one ask-whether-applied nudge with inline Telegram buttons (`app:<id>:<STATUS>` callbacks persist the answer; dashboard `POST /api/v1/applications/answer` mirrors it); NOT_APPLIED/NOT_INTERESTED suppress post-application follow-ups, drafts, and reminders. A strict list gate suppresses FORM/KYC/REGISTRATION notify + drafts + reminders when an attached candidate list (CSV/XLSX/PDF/OCR'd image) does not contain the student. Openings, academic/venue events, and the digest ride the legacy ladder unchanged; reviewer follow-up guidance is prompt-context only (no hard gate, owner choice 2026-09-22).
- **Rationale:** Owner report: repeated post-application-style alerts (e.g. Mars.AI account-creation) for companies never applied to, including messages carrying lists without the student's name. Eligibility records list-membership, which is not application — the missing concept.
- **Consequence:** Migration `20260922_0001`; `pia_worker/applications/`, `notify/classify.py`, `notify/buttons.py`; gates in `jobs/notify.py` (event + escalation), `jobs/propose_actions.py`, ask buttons on `notify_eligibility`; `telegram_ask.py` callback intake; `get_applications` agent tool + reviewer prompt; `routes/applications.py`; 05_Backend_Schema §2/§3.

### DEC-012 — Notification pipeline deep fix: context-first, attachment-honest, student-readable (2026-09-23)
- **Decision:** Rebuild the notify path around a normalized `ResolvedContext` (subject, topic, verification, deadline truth, recommendation, evidence) instead of raw classification fields. Message topics live in pure Python (`notify/topics.py` — OPPORTUNITY/REGISTRATION/SHORTLIST/INTERVIEW/ONLINE_TEST/ASSESSMENT/DOC_SUBMISSION/SURVEY/ADMIN/DEFAULTER_CHECK/TRAINING/INFO/SYSTEM_TEST/UNKNOWN); watcher/dry-run chatter classifies GENERAL/IGNORE and never enters the pipeline. Subjects resolve company → attachment filename → ALL-CAPS topic token → `📋 Placement Update` fallback (never "General", never invented, never minted). Attachment verification is a strict derived state (USER_PRESENT/USER_ABSENT/NOT_AVAILABLE/PARSE_FAILED/NOT_RELEVANT) — missing files never become verdicts. Downloads fail fast with codes (`base64_missing`, `evolution_media_not_stored`) while transport errors retry with backoff; a proven Evolution `getBase64FromMediaMessage` refetch recovers pre-base64 rows when the message survives in the Evolution store, plus `retry_attachment`/`reap_stuck_attachments` maintenance. Watchdog internals go to logs + optional `ADMIN_TELEGRAM_CHAT_ID`, never the student channel. Deadlines carry explicit truth (UPCOMING/TODAY/URGENT/EXPIRED/UNKNOWN; past-due never "TODAY"; moved deadlines re-arm via reschedule edges). Templates are student-first (ACTION/INFO/OPPORTUNITY/stage/EXPIRED/VERIFY); the digest is categorized with a per-day idempotency anchor; reminder-shaped actions merge canonically and deltas render as updates. Switches: `NOTIFICATION_ENABLED`, `DIGEST_ENABLED`, `ATTACHMENT_ANALYSIS_ENABLED`, `ATTACHMENT_RETRY_ENABLED`, `TEST_MESSAGE_FILTER_ENABLED`, `DEBUG_NOTIFICATIONS_ENABLED` (all default-on except debug).
- **Rationale:** Owner report with verbatim examples — semantically useless "General — REGISTRATION" alerts, unverified defaulter-list conclusions, DLQ tracebacks on the student channel, stale "TODAY" deadlines, repeated digest lines, duplicate BANGMETRIC alerts, test-noise leakage.
- **Consequence:** `notify/{topics,subject→companies/subject,context,buttons}`, `media_refetch.py`, gates + templates + digest in `jobs/notify.py` + `notify/render.py`, `deadline_status` in `notify/decide.py`, reschedule edges in `states.py`, `upsert_deadline` re-arm, `process_message.py` download rework + reap/retry, `watchdog.py` routing, `ai/rules.py` noise filter, settings + env.example flags, `tests/integration/test_notification_flow.py`, 05_Backend_Schema notes (no schema change — derived states only).
